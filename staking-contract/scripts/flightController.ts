...
const deployFlightController = async (
  ...
  const flightController = await FlightController.deploy(owner.address, aircraftNFTAddress, airlineCoin.address)
  ...
  const flight = await flightController.connect(otherAccount).flightDetails(otherAccount.address)
  if (!flight.startTime.isZero()) {
    ...
    try {
      ...
      const flightControllerContract = await ethers.getContractAt('FlightController', flightController.address)

      // Get the emitted event using the event filter
      const eventFilter = flightControllerContract.filters.FlightCompleted(null, null, null)
      const [event] = await flightControllerContract.queryFilter(eventFilter, txReceipt.blockNumber)
