...
contract StakingContract is Context {
  ...
  event Staked(address indexed user, uint256 amount, uint256 duration, uint256 endTime);
  ...
  function stake(uint256 _amount, uint256 _duration) external {
    ...
    require(_duration == 30 || _duration == 90 || _duration == 180, 'Invalid duration');
    require(stakes[_msgSender()].started == false, 'There is already a staking open');

    uint256 endTime = block.timestamp + _duration * 1 days;
    token.transferFrom(_msgSender(), address(this), _amount);

    stakes[_msgSender()] = Stake(_amount, _duration, block.timestamp, endTime, true);
  ...
  function calculateReward(Stake memory _stake) internal view returns (uint256) {
